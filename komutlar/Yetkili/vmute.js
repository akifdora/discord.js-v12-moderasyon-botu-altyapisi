...
user.roles.add(mute.id);
if (!msg.guild.members.cache.get(user.id).voiceChannel) msg.guild.members.cache.get(user.id).voice.kick()
let mutezaman = args[1]
if(mutezaman == "süresiz"){
  db.set(`vmuteli_${msg.guild.id + user.id}`, 'süresiz')
...
db.set(`vmuteli_${msg.guild.id + user.id}`, 'vmuteli')
db.set(`vsüre_${msg.mentions.users.first().id + msg.guild.id}`, mutetime)
